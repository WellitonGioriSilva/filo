...
const { addDoc, updateDoc, getDoc, getDocs, query, where, limit, serverTimestamp, runTransaction } = require('firebase/firestore');
...
async function getWaitingCount(shopId) {
  const q = query(col.position, where('barber_shop_id', '==', shopId), where('status', '==', 'waiting'));
...
router.post('/:shopId/join', auth('client'), async (req, res) => {
  try {
    ...
    res.status(201).json({ id: created.id, number, status, estimateMinutes: estimate });
