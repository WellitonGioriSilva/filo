...
const { addDoc, updateDoc, getDoc, getDocs, query, where } = require('firebase/firestore');
...
router.post('/', auth('barber'), async (req, res) => {
  try {
    ...
    if (!name || !addres || !avgServiceTime) return res.status(400).json({ error: 'Missing fields' });
